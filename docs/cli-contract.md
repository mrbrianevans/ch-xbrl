...
| Extension-less remote | `http(s)` URL whose path has **no** recognised archive or instance extension (e.g. Companies House `/document?format=xhtml`) | One streaming GET (redirects followed). The body is sniffed like stdin: XML/XHTML, tar, or tar.zst. **Zip is refused** so bulk `.zip` URLs keep HTTP range access — pass a URL that ends in `.zip`. `source_file` is the `Content-Disposition` filename when present, else the URL basename. Known extensions **do not** sniff (no extra work). |
