...
| Extension-less remote | `http(s)` URL whose path has **no** recognised archive or instance extension (e.g. Companies House `/document?format=xhtml`) | One streaming GET (redirects followed). Format is taken from a filename in `Content-Disposition` or the final URL query `response-content-disposition` (S3). Magic sniff is only the fallback if that filename is missing or has no recognised extension. **Zip is refused** so bulk `.zip` URLs keep HTTP range access — pass a URL that ends in `.zip`. `source_file` is that filename when present, else the URL basename. Known path extensions **do not** use this path (no extra work). |
